...
def benchmark(translator, num_types=10, num_ids_per_type=10, num_updates=10, use_time=False, use_geo=False):
    ...
    results = {}
    entities = create_random_entities(
        num_types, num_ids_per_type, num_updates, use_time=use_time, use_geo=use_geo)

    # Insert 1 entity
    res = timeit.timeit(
        partial(translator.insert, entities=entities[:1]), number=1, globals=globals())
    results[BM_INSERT_1E] = res

    # Insert N entities
    n = min(1000, num_types * num_ids_per_type * num_updates//10)
    res = timeit.timeit(partial(translator.insert,
                                entities=entities[1:n]), number=1, globals=globals())
    ...
    res = timeit.timeit(partial(translator.query, entity_type=entity_type,
                                entity_id=random_id), number=1, globals=globals())
    results[BM_QUERY_NA1E] = res

    # Query 1 attr of N entities
    res = timeit.timeit(partial(translator.query, attr_names=[
                        'attr_str']), number=1, globals=globals())
    ...
    res = timeit.timeit(partial(translator.average,
                                attr_name="attr_float"), number=1, globals=globals())
